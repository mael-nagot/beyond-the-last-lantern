...
**Purpose:** A live instance of an item — points at an `ItemData` and tracks per-instance state (`stack_count`, `durability`, `key_id`, `tint`). Stackable items share an instance with `stack_count > 1`; equipment with durability gets one instance per piece. Use `ItemInstance.create(data, count)` to build one. `can_stack_with(other)` checks if two instances can merge — for keys, two instances built from the same key `.tres` but with different `key_id`s do NOT stack (they unlock different doors). `get_key_id()` returns the per-instance `key_id` if non-empty, else falls back to `data.key_id` — auto-generated lock ids land on the instance, designer-baked ones live on the data. `tint: Color` (default `Color.WHITE` = identity multiplier) is applied as `modulate` in every renderer (`ItemBar` slot button, `DungeonView` floor sprite, `LootPopup` slot icon) — `KeyDoorSpawn` placement uses this so keys for different locks but the same `.tres` get slightly different shades.
