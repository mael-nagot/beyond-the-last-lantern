...
**Inputs:** WASD/arrows + Q/E for movement; F to pick up items on the current tile. **Map gating (Phase 6 polish):** `_on_move` (movement-pad signals) and `_input` (keyboard) both early-return via `_is_map_blocking_input()` while `MapPopup.is_open()` is true, so the player can't accidentally walk / pick up / fire debug keys while the map is up.
**Debug keys:** F1 spawns a Health Potion into the item bar; F2 damages every party member by 10 HP for testing heal; F3 reveals the entire map. (All three are gated by the map block alongside movement.)
...
**Key methods:** setup(gen, map_data), open(), close(), is_open() (true while visible — including the close-animation tail; Game.gd uses this to gate movement / pickup / debug input), update_player(pos, facing), redraw() (force a redraw of the map after external state changes — used by F3 debug reveal)
