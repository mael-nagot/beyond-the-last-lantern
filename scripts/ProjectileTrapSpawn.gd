...
# projectile launcher from this spawn. Rolled per segment, per spawn.
# 0 disables the corridor pass entirely. Used in Subtask C1.
@export_range(0.0, 1.0) var corridor_chance: float = 0.0
# Maximum launchers placed in a single corridor segment when the
# segment is chosen. Usually 1 — a corridor with two launchers feels
# like crossfire and undermines the "fire toward the junction" escape
# guarantee. Designers can crank this higher for set-piece moments.
...
# launcher from this spawn. 0 disables the room pass. Code wiring
# lands in Subtask C5; the field is defined now so the resource
# doesn't change shape later.
@export_range(0.0, 1.0) var room_chance: float = 0.0
# Maximum launchers placed in a single room when the room is chosen.
