...
@export_group("Damage")
# Applied to all 3 party members when the trap fires. Values <= 0 are
# treated as no-damage (useful for harmless prop traps).
@export var damage: int = 5

@export_group("Timing — STEP")
# How long spikes stay extended after a step trigger.
@export var step_activation_duration: float = 0.5
# Lockout period after spikes retract — the trap won't re-trigger even
# if the player steps off and back on. 0 = no cooldown (re-triggers
# every entry).
@export var step_cooldown_duration: float = 1.5

@export_group("Timing — TIMED")
# How long spikes stay extended each cycle.
@export var timed_up_duration: float = 1.0
# How long spikes stay retracted each cycle.
@export var timed_down_duration: float = 2.0
# Per-instance phase shift (seconds added to the timer at spawn) so
# multiple traps don't activate in lockstep. The placer applies a
# pseudo-random offset within [0, timed_up + timed_down) on top of
# this base value.
@export var timed_initial_offset: float = 0.0

@export_group("Art")
# Single hole sprite — top-down view, transparent background. Rendered
# as a FLAT MeshInstance3D quad on the floor. Drawn once per grid cell
# in the trap's grid_cols × grid_rows pattern.
@export var holes_sprite: Texture2D
# Single spike sprite — side view, transparent background. Rendered as
# a vertical Sprite3D BILLBOARD_FIXED_Y, anchored bottom-on-floor.
# Visible only when the trap is EXTENDED.
...
# spike base to match the hole opening regardless of how the artist
# proportioned the source PNG. Implemented via `Sprite3D.scale.x`
# so the sprite stretches horizontally without affecting its
# vertical pixel scaling.
@export var spike_world_width: float = 0.0
# Cell-relative size of each individual hole (world units). 0 = derive
# from the texture's natural pixel size against the cell.
@export var hole_world_size: float = 1.0
# Vertical offset for the spikes' bottom anchor, in case the artist
# left transparent padding at the bottom of `extended_sprite`.
@export var y_offset: float = 0.0
# Cell-relative shift applied to the SPIKE subtree (only — the floor
# holes stay anchored to the cell centre) toward whichever cardinal
# side the player is currently on. Mirrors `ObjectData.lean_toward_player`
# but only nudges the vertical sprites, so the spikes feel like they
# protrude near the player while the holes remain a cell-wide
# permanent feature. 0.0 = no lean (default — spikes centred over
# their holes). ~0.5–1.0 = noticeable forward push when viewing the
# trap from an adjacent cell. Refreshed only on player turn (same
# trigger as the chest lean) — refreshing on move would visibly snap
# the spikes mid-step.
@export var spike_lean_toward_player: float = 0.0

@export_group("Layout — Per Cell")
# Number of spike+hole columns / rows distributed across one grid cell.
...
# 0.0 = grid touches the cell edges; 0.5 = bunched at centre. Default
# 0.3 keeps spikes well inside the tile so the player can clearly read
# the cell boundary.
@export_range(0.0, 0.5) var grid_inset: float = 0.3

@export_group("Audio")
# Played on RETRACTED → EXTENDED transition.
@export var activate_sound: AudioStream
# Played on EXTENDED → RETRACTED transition.
@export var deactivate_sound: AudioStream
# Linear distance falloff (world units) for the trap's spatial audio.
# Beyond this radius the sound is silent. Used by AudioStreamPlayer3D's
# `max_distance`. Only meaningful for TIMED traps — STEP traps fire
# right at the player's feet, so they play through SoundManager's
# non-spatial pool instead.
