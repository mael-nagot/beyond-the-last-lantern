...
## Translation key for the teleporter's display name. Use
## `get_display_name()` to fetch the translated text.
@export var name_key: String = ""
## Translation key for the teleporter's description tooltip.
...
## `default_animation`. Either `sprite` or `frames` should be set;
## if both are set the renderer prefers `frames`.
@export var frames: SpriteFrames
## Sprite height in world units. The renderer derives `pixel_size`
## from `world_height / texture_height` (same pattern as chest /
## lever / spike sprites) so designers control real-world size
## without fiddling with per-sprite scale.
@export var world_height: float = 1.8
## Vertical offset added to the sprite's centre position (which is
## anchored at `world_height * 0.5` above the floor so the sprite
## sits ON the floor, not floating). Useful for nudging a rune
## circle upward if the source art has transparent padding at the
## bottom.
@export var y_offset: float = 0.0
## Sprite opacity in [0, 1]. 1 = fully opaque, <1 = semi-transparent
## (sells "this is a magical projection, not a physical prop").
## Applied via the Sprite3D / AnimatedSprite3D `modulate.a`. Note:
## `alpha_cut = ALPHA_CUT_OPAQUE_PREPASS` still hard-cuts the
## texture's transparent BACKGROUND — the sprite outline stays
...
## [0, 1]. 0 = static (no pulse), 0.5 = ±50% swing, 1.0 = full
## off-to-brighter-than-base swing. The sprite cycles between
...
## phase — both endpoints of the SAME pair stay in sync (one magic),
## but different pairs feel independent. ~1.6 (a bit over PI/2)
## gives a noticeable offset without looking like a chase.
...
## subtle "the rune is leaning toward you", 0.6+ = obvious "the
## rune is greeting you". Recomputed on player turn (NOT on
## movement, same as the chest lean), so the rune doesn't visibly
## snap mid-step.
@export_range(0.0, 1.5, 0.05) var lean_toward_player: float = 0.0

@export_group("Light")
## Tint of the warm glow under the rune circle. Only used when
## `light_energy > 0`.
@export var light_color: Color = Color(1.0, 0.7, 0.4)
## Brightness of the omni light spawned at the rune. 0 disables the
## light entirely (no OmniLight3D spawned — useful when the sprite
## is bright enough on its own).
@export var light_energy: float = 1.0
## Radius (world units) of the omni light's falloff. Larger values
## bleed the glow further across the floor and walls.
...
## Reserved for Phase C: played once when the player tries to walk
## into a sealed wall (audible feedback that "there's no way through
## here, find the rune circle"). Unused in Phase A — sealed walls
## don't exist yet.
