...
#### Phase 4 polish — Outdoor biomes (no walls) ✅
Some biomes are open-air — a sparse forest, a beach — and have no wall geometry. The impassable space outside the walkable area is filled with billboarded "filler" sprites (trees, rocks, bushes) instead, with the sky visible past them. Player movement is unchanged — only the visuals.
1. ✅ `FillerData.gd` Resource — `texture`, `world_height`, `y_offset` (mirrors the Sprite3D pattern: `pixel_size = world_height / texture_height`)
2. ✅ `FillerSpawn.gd` Resource — biome pool entry: `fillers` array (uniform pick), `density_min/max` per blocked cell, `jitter_radius` (sub-cell scatter as a fraction of CELL_SIZE), `front_row_bias` (pulls sprites in WALL cells that border FLOOR cells toward the floor-adjacent edge so the wall line reads clearly — without it, random jitter creates apparent walkable gaps), `border_ring_depth` (cells beyond the grid edge to also seed for horizon depth), `scale_min/max` (per-instance size variety so every tree isn't the same height)
3. ✅ `FillerInstance.gd` RefCounted — runtime record (data, cell, cell_offset, scale). `cell` may be OUTSIDE the grid for border-ring fillers
4. ✅ `BiomeData` additions: `outdoor_mode: bool`, `filler_spawns: Array[FillerSpawn]`, `filler_floor_textures: Array[BiomeTextureEntry]` (under-filler floor; falls back to `floor_textures` when empty), `outdoor_floor_extent: int` (cells of floor past the grid edge), `sky_color: Color` (alpha > 0 = use as flat background), `sky_material: Material` (when set, wrapped in a Sky resource and used as the background — clean path to swap in a panorama texture later)
5. ✅ `LevelGenerator._place_fillers()` — runs last in the pipeline; no-op when `outdoor_mode = false`. Iterates the WALL-cells + border-ring grid, samples density per cell, picks sprites, applies jitter + scale, deterministic under a seeded RNG
6. ✅ `DungeonView._build_mesh` skips wall quads and the ceiling pass when `biome.outdoor_mode = true`; `_build_outdoor_floors()` extends floor geometry past the walkable area (under fillers + `outdoor_floor_extent` cells outside the grid) so trees don't appear to float on the sky; `_build_fillers()` instantiates one `Sprite3D` per filler under `_fillers_root` with `BILLBOARD_FIXED_Y` (always faces camera, stays vertical). `_apply_biome_sky()` swaps the WorldEnvironment background per biome and restores the scene defaults when leaving an outdoor biome
7. ✅ Tests: unit (`test_filler_data.gd`, `test_filler_spawn.gd`, `test_filler_instance.gd`) + integration (`test_level_generator.gd` — fillers off-when-indoor / pool-empty / one-per-WALL-cell at fixed density / never-on-floor / border-ring extends outside / deterministic under same seed)
8. ✅ SparseForest biome (`assets/biomes/sparse_forest.tres`) + `assets/filler/sparse_forest_tree.tres` proof-of-concept. Wall decorations, projectile traps, and secret walls are conceptually banned in outdoor biomes (they need wall geometry to attach to); validation gate is deferred — biomes leave those arrays empty by convention
...
#### Phase 7 polish 2 — Items visible at the player's feet ✅
Floor-item Sprite3Ds previously sat dead-centre on their cell. When the player walked onto a tile holding items, the sprites ended up directly under the camera (eye height 1.8 vs item centre ~0.3) and fell outside the forward view cone — invisible. The `PickupPrompt` button told you something was there, but you couldn't see what.
1. ✅ `DungeonView.ITEM_ON_TILE_FORWARD_OFFSET` (1.5 world units) — the per-stack shift along the player's facing direction when the player stands on the cell. Other cells stay centred so a pile across the corridor still reads as one cluster.
2. ✅ `DungeonView._item_world_position(grid_pos, stack_index, inst)` — pure positioning helper. Checks `grid_pos == _current_grid_pos` to decide whether to apply the shift; shift axis is `_current_facing` so a turn rotates the stack around the cell centre.
3. ✅ `DungeonView._item_sprites: Dictionary[Vector2i → Array[Sprite3D]]` — lookup table populated by `_build_items` (cleared on rebuild) so `_refresh_item_positions` doesn't have to walk the scene tree.
4. ✅ `_refresh_item_positions(animated)` wired into `set_initial_facing` (animated = false — initial snap on level start), `move_camera_to` (animated = true), and `rotate_camera_to` (animated = true). When animated, position changes tween over 0.12s in parallel with the camera tween so items glide between centred and shifted positions instead of popping. First-pass instant snap felt glitchy when stepping on/off item tiles — the tween smooths it out without changing the destination.
5. ✅ `_item_tween` member tracks the active tween; killed on every refresh so consecutive moves / turns don't fight each other for the sprite's position. Same defensive pattern as `_move_tween` in `move_camera_to`.
6. ✅ No new tests — the positioning helper lives on a Node3D (DungeonView) rather than a pure-logic Resource / RefCounted, so it falls under the visual-rendering exclusion in `tests/README.md`.
...
##### Subtask C — Projectile trap system ✅ (C1–C5 shipped; C6 deferred)
...
Split into six sub-PRs so each one has something testable in-engine before merging. C1–C5 shipped (with a follow-up that switched TIMED traps from launch-period to break-duration semantics so the designer's tuning value matches the visible empty-corridor time, and a follow-up that requires PRESSURE_PLATE corridor launchers to have a junction at a non-endpoint position of the projectile path for player-fairness). C6 is deferred to the status-effect gameplay phase — see its entry below.
...
###### Subtask C5 — Room placement + wall-face exclusivity vs decorations ✅ (code; awaits manual playtest)
1. ✅ `_place_room_projectile_traps(spawn)` — iterates `_room_rects`, rolls `room_chance` per room, builds candidate (cell, wall_dir, path) pool via `_projectile_launch_path_in_room()`. Same revalidate-before-commit pattern as the corridor pass: re-check face registry + path overlap + spreading rule just before committing each candidate (an earlier winner in the same loop may have claimed the face or laid down an overlapping path). Per-room launcher cap = `ProjectileTrapSpawn.room_max_per_room`. Host-cell exclusions match the corridor pass (no entrance/exit, no blocking object, no spike trap).
2. ✅ `_projectile_launch_path_in_room(cell, wall_dir, room)` — variant of the corridor path validator that requires the FULL projectile path to stay inside the same `Rect2i`. Escaping through a doorway into a corridor or an adjacent room makes the projectile unavoidable outside the controlled space, so any path cell falling outside the room rect rejects the candidate. Same chest/lever/exit/path-overlap fairness rules as corridor placement; no junction-reachability rule (rooms have no junctions in the corridor sense — the player escapes by sidestepping inside the room or stepping out through a doorway).
3. ✅ Wall-face exclusivity — projectile launchers and wall decorations share `_wall_faces_used` (registry from C1). The room pass calls into the same registry, so a room-placed launcher's face is blocked from decorations downstream and vice versa.
4. ✅ PRESSURE_PLATE plates inside rooms — relaxed `_pick_plate_cell_for_room(path, launcher_cell, data)`: keeps the "on path" + "≥ `min_plate_to_launcher_distance`" rules from the corridor variant, drops the `max_plate_to_junction_distance` constraint. PRESSURE_PLATE room launchers that can't satisfy the relaxed rules are still rejected (no inert plates).
5. ✅ Tests: integration (`test_level_generator` extended — room placement lands at `room_chance = 1.0`, every placed launcher's host cell is inside some `_room_rects` rect, every projectile-path cell stays inside the host room rect, per-room cap honoured under `room_max_per_room = 1` with extra candidates available, room placements mount on actual walls, no path overlap when corridor + room passes run together, decorations never share a face with a room-placed launcher, PRESSURE_PLATE room launchers land even when `max_plate_to_junction_distance` would block the corridor rule, spreading rule still applied across room launchers).

###### Subtask C6 — Variants + status effect data populated ⏸ DEFERRED to the status-effect gameplay phase
The C1–C5 system already supports an arbitrary number of variants with no code branches — adding poison-dart / ice-shard / extra-fireball `.tres` files is pure content work, and the `status_effect_id` / `_duration` / `_magnitude` data fields populate nothing useful until status-effect gameplay exists. Authoring the variants now would mean tuning numbers (damage / speed / effect magnitudes) blind to the gameplay system that will read them, then re-tuning later. Defer the whole subtask: when the status-effect gameplay phase lands (party-side DOTs / slows / burns wired across all damage sources — traps, enemies, environmental hazards), author the variants then with real feedback on what the numbers feel like in combat. Carry over from this subtask's original scope:
1. Additional `.tres` variants — `projectile_trap_poison_dart_timed.tres`, `projectile_trap_ice_shard_timed.tres` (the fireball-pressure-plate variant already exists from C4 art work). Each tunes sprites, audio, world_height, speed, damage, and status effect fields independently.
2. Populate `status_effect_id` / `_duration` / `_magnitude` on every projectile-trap `.tres` once the gameplay system defines the id taxonomy.
3. Variants spread across biomes via `projectile_trap_spawns`.
4. Localization: `projectile_trap.{poison_dart,ice_shard,...}.{name,description}` rows added to `strings.csv`.
5. Tests: resource-load sanity (each `.tres` loads, fields parse, status data round-trips); status-effect application tests once gameplay exists.
...
- Projectile-trap variants (C6 deferred — see above) author alongside the status-effect gameplay phase so the numbers can be tuned with real combat feedback
...
#### Task 2d — Wall switches + hidden doors ✅ (code; awaits manual asset/biome wiring)
A new puzzle mechanic: a wall-mounted switch toggles a door that can look indistinguishable from a regular wall (true secret passage). Sibling to Task 2b lever-door pairs — same gameplay shape (one trigger, one controlled door, reachable-with-door-closed contract) but the switch lives on a wall FACE (not a floor cell) and the door is typically rendered as a regular wall when closed and disappears entirely when open. The switch is the only visible cue and the only interaction surface.
1. ✅ `ObjectData` extended — `appears_as_wall: bool` (DOOR only; renders as biome wall geometry when closed, void when open, non-clickable; reusable for future quest-gated / trade-cost-gated hidden doors) and `hide_when_active: bool` (WALL-SWITCH only; only honoured for on-door placements — the switch + click area vanish after a linked door opens, turning the puzzle into a one-shot).
2. ✅ `WallSwitchInstance.gd` (RefCounted, extends `ObjectInstance`) — wall-face anchored switch. `view_cell` is the floor cell the player stands in to see and click the switch, `view_side` is the cardinal direction toward the wall (SIDE_NORTH/EAST/SOUTH/WEST = 0/1/2/3), `along_offset` is a signed offset in WORLD UNITS along the wall direction (random sign at placement). `linked_doors` cross-link with the new `DoorInstance.linked_wall_switches`. `is_on_door()` is true iff `view_cell` is a linked door's endpoint AND `view_side` points across the door's edge. `get_visual_opened()` mirrors LeverInstance — true when any linked door is open — drives the sprite swap. `should_hide()` combines the data flag + on-door + door-open conditions.
3. ✅ `WallSwitchedDoorSpawn.gd` (Resource) — biome-level entry. `min_wall_distance` / `max_wall_distance` in Manhattan CELLS (default 0/0 = on-door always); the placer samples uniformly across non-empty distance buckets, so `max = 1` flips 50/50 between on-door and adjacent wall, `min = 2, max = 4` puts the switch on a nearby real wall. `along_offset_min` / `along_offset_max` in WORLD UNITS (consistency with `world_height` / `lean_toward_player` / etc; cell is ~4.6 world units wide so usable values land 0.0–2.0). `door_must_gate_content` default false (decorative wall-switched doors allowed). Sibling to `LinkedObjectSpawn` (lever-door clusters).
4. ✅ `DoorInstance.linked_wall_switches: Array` — back-link populated at placement so Game.gd can refresh switch sprites after a door toggle (mirrors the role `linked_levers` plays).
5. ✅ `BiomeData.wall_switched_doors: Array[WallSwitchedDoorSpawn]` — placed AFTER lever-door clusters and BEFORE key-doors / items / secret walls.
6. ✅ `LevelGenerator._place_wall_switched_doors()` — for each spawn, attempts `count_min..count_max` pairs. Each pair picks a 1-wide-corridor edge for the door (same eligibility as decorative + lever-locked doors), then builds switch candidates bucketed by Manhattan distance to a door endpoint. Distance 0 emits two candidates per door (one per endpoint, view_side across the edge). Distance > 0 iterates floor cells within range whose `view_side` faces a real WALL cell; switches that would float on a floor↔floor boundary are rejected. `along_offset` is rolled per placement (abs in `[min, max]`, sign 50/50). Chain reachability extended: wall-switched doors are excluded from the auto-open set initially AND promoted to openable once any linked switch's view cell is reachable (mirrors the lever fixed-point loop). Atomic rollback on failure — no orphan halves.
7. ✅ `DungeonView` rendering — new `WallSwitchesRoot` Node3D; one Sprite3D per switch anchored at the wall face midpoint (offset along the wall tangent by `along_offset`, Y-rotated so the sprite faces back at the player who stands in `view_cell`), with a child Area3D + box collider for click pickability. `should_hide() == true` skips rendering. Doors with `data.appears_as_wall` branch to `_make_hidden_door_node`: two coplanar MeshInstance3D wall quads (one per side, opposite normals) sharing a single material picked deterministically via `BiomeTextureEntry.pick_for(biome.wall_textures, ...)` so both faces match. Opened hidden doors render nothing. `rebuild_doors()` cascades to `rebuild_wall_switches()` so a door toggle refreshes its switches in lockstep.
8. ✅ `Game._on_object_clicked` — `WallSwitchInstance` dispatched to `_toggle_wall_switch`: flips every linked door's `opened` directly (TOGGLE semantics — different from levers which compute the door from all linked levers' pulled states). Plays the switch's `interact_sound` immediately, then the door's `interact_sound` after `_LEVER_DOOR_SOUND_DELAY` (only when a door's state actually changed — cause-and-effect feel). Rebuild path cascades through `rebuild_doors()` → `rebuild_wall_switches()`.
9. ✅ `MapPopup` — slab pass skips `appears_as_wall` doors; new hidden-door pass draws a wall LINE on the edge when closed and nothing when open (player can't tell it's a door from the map until they open it). Switch dash pass draws a short tick mark perpendicular to the wall, centred at the wall face midpoint + `along_offset` (converted from world units to map pixels), brown when closed (no linked door open) / blue when open. Switches with `should_hide() == true` skipped, mirroring the 3D view.
10. ✅ Localization: `object.door_forest_hidden.{name,description}`, `object.switch_forest.{name,description}` rows in `strings.csv`.
11. ✅ Tests: unit (`test_wall_switch_instance.gd` — create, dir_for_side, toggle, get_visual_opened mirrors linked doors, is_on_door endpoint matching, should_hide scope rules; `test_wall_switched_door_spawn.gd` — defaults, count/distance/offset settable). Integration (`test_level_generator.gd` extended — count parity, switch ↔ door back-link, switch reachable with linked door closed via the new chain simulator branch, distance-0 lands on door, distance-2 is off door + within Manhattan range, view_side points at a real WALL cell for off-door placements, along_offset in configured range, appears_as_wall propagates onto the placed DoorInstance, no orphan halves across 5 seeds).
12. ⏳ Manual asset work (developer): `res://assets/objects/switch_forest.tres` (`ObjectData` with switch sprite pair — `closed_sprite = idle`, `opened_sprite = activated`; `world_height ≈ 1.0–1.5`; optional `hide_when_active = true` for one-shot puzzles where the switch sits on the door panel). `res://assets/objects/door_forest_hidden.tres` (DOOR `ObjectData` with `appears_as_wall = true`, `blocks_movement = true`; `closed_sprite` / `opened_sprite` can be null — the renderer uses the biome wall material instead). Add a `WallSwitchedDoorSpawn` entry to `res://assets/biomes/forest.tres` referencing both, configure `min/max_wall_distance` + `along_offset_min/max` to taste.
...
#### Task 8 — Spinners ✅ (code; awaits manual asset/biome wiring)
Floor tile that rotates the player N times when stepped on, disorienting them (loses bearing of which way the entrance is). Light comedic / puzzle effect; pairs well with biomes where the map is partially obscured. **Architecture diverged from the initial sketch:** spinners ended up as a SEPARATE system from `ObjectData` (parallel to spike traps) so the trap-style configuration model (corridor cluster + room density + scattered passes) could be reused verbatim. Each spinner's direction + rotation count is rolled ONCE at placement and locked for life — a specific spinner always spins the same way, but different spinners (even those sharing the same SpinnerData) can roll different values.
1. ✅ `SpinnerData.gd` Resource — `min_spins`/`max_spins` rotation count range, `direction` enum (CLOCKWISE / COUNTER_CLOCKWISE / RANDOM as a *policy*), `spin_step_duration` (per-90°-turn duration, floored at 0.12s by Game.gd to leave room for the rotate tween), floor decal art (`decal_sprite`, `decal_world_size`, `y_offset`), `visual_rotation_degrees_per_second` for the always-on swirl, optional `spin_sound` per 90° turn (falls back to the global turn sound).
2. ✅ `SpinnerInstance.gd` RefCounted — placement record holding `data`, `cell`, plus a resolved-once-at-placement `direction` (concrete CLOCKWISE or COUNTER_CLOCKWISE — never RANDOM) and `rotations` (rolled in `[data.min_spins, data.max_spins]`). `SpinnerInstance.create(data, cell, direction, rotations)` clamps rotations to >= 1 (so a misconfigured 0 still does something) and defensively collapses invalid direction values to CLOCKWISE.
3. ✅ `SpinnerSpawn.gd` Resource — mirrors `TrapSpawn` exactly: scattered (`count_min`/`count_max` + `placement` flags + `min_distance_to_other_spinner` with graceful degrade), corridor cluster (`corridor_segment_chance`, `corridor_spinners_per_run_min/max`), room density (`room_chance`, `room_coverage_min/max_percent`, `room_min_spacing`, `allow_mixed_room_spinners`).
4. ✅ `GridCell.spinner: SpinnerInstance` (third parallel slot alongside `object` and `trap`). Spinners don't block movement.
5. ✅ `BiomeData.spinner_spawns: Array[SpinnerSpawn]` — placed AFTER projectile traps (so plate cells are known) and BEFORE wall decorations.
6. ✅ `LevelGenerator._place_spinners()` + helpers. Exclusivity rules (per user spec): a spinner never shares a cell with a trap (`cell.trap`), chest / lever (`cell.object`), floor item (`cell.items`), projectile-trap pressure plate (collected via `_gather_plate_cells()` from `projectile_traps`), or entrance / exit. The corridor cluster pass reuses `_detect_corridor_segments` + `_gather_run_in_segment` from the trap system; the room pass reuses the coverage-percentage roll + `room_min_spacing` model. `_commit_spinner_at` rolls each instance's fixed direction + rotation count once at placement using the seeded RNG.
7. ✅ `DungeonView` rendering — `SpinnersRoot` Node3D. Each spinner gets a `Node3D` parent at the cell with a flat `MeshInstance3D` `QuadMesh` child laid horizontal (rotated -π/2 around X) textured via the shared `_trap_floor_material_cache` (alpha-scissor + NEAREST filter). `_update_spinner_rotations(delta)` advances each root's `rotation.y` per frame at `data.visual_rotation_degrees_per_second × delta` (sign matches the instance's CW/CCW), `fmod`'d into `[0, 2π)` for precision. Rotating the ROOT (not the decal) keeps the world-Y spin clean of the decal's -90° X tilt. Designers can ship "invisible" spinners by leaving `decal_sprite` unset.
8. ✅ `MapPopup` glyph — small purple swirl arc with a comma-tail tip, sweep direction matches the instance's resolved CW/CCW. Gated behind `MapPopup.debug_show_spinners` (default `true` for dev validation, flip off for shipping) so the convention matches the other trap-system markers (`debug_show_traps`, `debug_show_projectile_traps`).
9. ✅ `Game._on_player_entered_cell` — runs the spinner trigger last (after spike-trap + plate branches). The armed-cell guard (`_spinner_just_triggered_cell`) prevents re-firing when the player turns voluntarily while standing on the spinner; stepping off ANY direction clears the latch so step-off-and-back-on rearms.
10. ✅ `Game._trigger_spinner` — chained-rotation driver. Flips `_spinner_spinning = true` (folded into `_is_world_paused()` so player input drops cleanly across `_on_move` and `_input`), dispatches `spinner.rotations` 90° turns to PlayerController via `turn_right(false)` / `turn_left(false)` (the new `play_sound` arg suppresses the global turn rustle so it doesn't double up the spinner's own whoosh), `await`s `spinner.data.spin_step_duration` (floored at 0.12s so the rotate tween settles between steps) per turn. Timer uses `process_always = false` so an opened modal cleanly pauses the spin.
11. ✅ `PlayerController.turn_left(play_sound = true)` / `turn_right(play_sound = true)` — new optional parameter (default `true`) lets spinner spins suppress the per-turn sound. Backwards-compat preserved for every existing caller.
12. ✅ Localization: `spinner.forest.{name, description}`.
13. ✅ Tests:
    - Unit (`test_spinner_data`, `test_spinner_instance`, `test_spinner_spawn`): default values, direction enum shape (RANDOM only on SpinnerData, never on SpinnerInstance), placement-flag matrix, `uses_corridor_clusters` / `uses_room_density` predicates, `create` clamping + invalid-direction defence.
    - Unit (`test_grid_cell` extended): default `spinner` is null, floor with spinner is not blocked.
    - Integration (`test_level_generator` extended): empty-pool no-op, scattered count within `[min, max]`, flat-list ↔ `cell.spinner` consistency, entrance/exit excluded, floor-only landing, corridor-only placement, exclusivity vs traps / chests / floor items / pressure plates (across multiple seeds for plates because PRESSURE_PLATE placement isn't guaranteed on every seed), instance `cell` field matches grid position, instance rotations within `[min_spins, max_spins]`, RANDOM policy yields concrete CW/CCW values, CLOCKWISE/COUNTER_CLOCKWISE policies always yield the expected direction, `min_distance_to_other_spinner` Manhattan spacing, corridor cluster chance=0 no-op + chance=1 lands in corridors, room chance=0 no-op + chance=1 lands inside rooms, `room_min_spacing` honoured within rooms, `allow_mixed_room_spinners=false` blocks the second spawn from sharing rooms.
14. ⏳ Manual asset work (developer): `res://assets/textures/objects/spinner_forest.png` (top-down swirl decal, ~128×128 transparent background), `res://assets/objects/spinner_forest.tres` (SpinnerData), optional `res://assets/sounds/spinner/spin_whoosh.ogg`. Add a `SpinnerSpawn` entry to `forest.tres` to validate placement in-engine (e.g. `room_chance = 0.5`, `room_coverage_min_percent = 15.0`, `room_coverage_max_percent = 30.0`).

#### Task 9 — Secret walls ✅ (code; awaits manual biome wiring)
Edge-based "fake walls" between two 1-wide-corridor floor cells. From the dungeon view the player sees a normal corridor wall (rendered with the biome's existing `wall_textures` so it's indistinguishable from the real thing); the movement system never blocks the edge, so a forward step "passes through". On the map the wall is drawn at reduced opacity (≈ 0.35 alpha multiplier on the regular wall colour) — an observant player notices the lighter line and tries walking through, while a casual glance reads it as a normal wall. The wall stays rendered after the player walks through — no discovery state — so a returning walk presents the same illusion. Placement reuses the door-edge eligibility (1-wide corridor, not entrance/exit, no chest underneath, edge not already used by a door or another secret wall).
1. ✅ `SecretWallSpawn.gd` Resource — `count_min`/`count_max`, `gate_mode: GateMode { NONE, ANY_CONTENT, LOOT_ONLY }` (default ANY_CONTENT), `min_distance_to_other_object`. NO ObjectData reference — wall textures come from the biome's regular pool so the secret wall is indistinguishable from any other corridor wall. `LOOT_ONLY` is stricter than just "must hide loot": it additionally REJECTS any edge whose sealing would gate the exit, a lever, or a key — secret walls in this mode are always pure side-rewards, never blockers on the main path. `ANY_CONTENT` mirrors the locked-door rule (any chest / lever / key / exit gated is acceptable).
2. ✅ `SecretWallInstance.gd` RefCounted — pure metadata (`cell_a`, `cell_b` canonically ordered, plus `canonical_pair` / `edge_key` / `axis` helpers mirroring `DoorInstance`). NO `discovered` flag — the wall always renders and the player always walks through.
3. ✅ `BiomeData.secret_wall_spawns: Array[SecretWallSpawn]` exposed for biome configs.
4. ✅ `LevelGenerator._place_secret_walls()` runs LAST among placement passes (after `_place_spinners()` and `_place_items()` so LOOT_ONLY validation can include floor items). `_candidate_edges_for_secret_wall()` mirrors `_candidate_edges_for_door`. `_secret_wall_gates_content(a, b, gate_mode)` extends the chain-reachability machinery with a new `extra_closed_edges` parameter on `_chain_reachable_from_entrance` / `_bfs_with_doors_state` — the BFS treats the secret-wall edge as a permanent wall and the placer rejects edges that don't actually gate something of the configured kind.
5. ✅ `DungeonView._build_mesh` — when iterating a floor cell's neighbours, also paint a wall quad on edges where `generator.get_secret_wall_at_edge(grid_pos, npos) != null` (in addition to the original "neighbour is WALL" rule). Texture picking flows through the same `BiomeTextureEntry.pick_for` pipeline + history dict, so the secret wall participates in spacing and weight rules like any other corridor wall.
6. ✅ `MapPopup` — iterates `generator.secret_walls`; for each whose EITHER endpoint is explored, draws a wall LINE on the edge with the regular wall colour but alpha multiplied by `SECRET_WALL_MAP_ALPHA` (= 0.7). Same geometry as the regular cell-wall lines so the secret wall looks like a real wall on the map until the player notices it's lighter. Pairs with `MapData.reveal_around` skipping the cell behind a secret wall (and its two adjacent diagonals, to cover the bent-corridor case) so the player only sees the faint wall — not the corridor continuation — until they walk through.
7. ✅ `PlayerController` — UNCHANGED. Secret walls never appear in `is_edge_blocked`, so movement just works.
8. ✅ Tests:
   - Unit (`test_secret_wall_spawn`): defaults, GateMode enum integrity.
   - Unit (`test_secret_wall_instance`): canonical_pair / edge_key / axis matching DoorInstance, edge_key string format matches DoorInstance.edge_key (so cross-registry lookups work).
   - Unit (`test_map_data` extended): `reveal_around` skips the orthogonal cell behind a secret wall + the two adjacent diagonals, opposite-side reveals stay intact, `reveal_around` from the far cell doesn't leak BACK through the wall.
   - Integration (`test_level_generator` extended): empty-pool no-op, endpoints are 1-wide corridor cells, endpoints orthogonally adjacent, `is_edge_blocked` never reports secret walls as blocking, secret walls never share an edge with a door, never on entrance / exit, `get_secret_wall_at_edge` order-independent, secret walls never on `GridCell.object`, ANY_CONTENT placements actually gate content, NONE mode commits placements without a gating requirement, LOOT_ONLY never gates the exit (multi-seed sweep), LOOT_ONLY always hides at least one loot cell, and a 25-seed stress sweep verifies the BRIDGE invariant — every placed wall has at least one endpoint that becomes unreachable when the edge is sealed (catches "wall on a loop" regressions).
9. ⏳ Manual biome wiring (developer): add a `SecretWallSpawn` to `forest.tres` (e.g. `count_min = 1`, `count_max = 2`, `gate_mode = LOOT_ONLY` to ensure each secret wall hides a chest or floor item) to validate the feature in-engine.

#### Task 10 — Walkable-area scenery ✅ (code; awaits manual biome wiring)
Trees, flowers, mushrooms, and rocks placed on FLOOR cells INSIDE the dungeon. Walkable scenery (flowers) lets the player step through; non-walkable scenery (trees) blocks the cell like a chest and is BFS-validated so the level stays solvable. Separate from outdoor `FillerData` (which lives on WALL cells of outdoor biomes). Three additive placement passes per spawn — per-dead-end chance, per-corridor-segment chance + coverage %, per-room chance + coverage % — plus a `min_distance_to_same` Manhattan spacing rule between two placements of the SAME scenery type.
1. ✅ `SceneryData.gd` Resource — `walkable` bool, `texture`, `world_height`, `y_offset`, `lean_toward_player` (centred for flat decorations; ~1.0–1.5 for solid objects so they read as 3D rather than a flat pixel cluster). Same Sprite3D billboard pattern as items / chests / fillers.
2. ✅ `SceneryInstance.gd` RefCounted — `data`, `cell`, `cell_offset` (per-sprite sub-cell jitter in fractions of CELL_SIZE), `scale` (per-sprite multiplier on `data.world_height`), `blocks_movement()` mirrors `not data.walkable`. Consumed by `GridCell.is_blocked` (a tree cell behaves exactly like a chest cell).
3. ✅ `ScenerySpawn.gd` Resource — biome-level entry: `scenery`, `dead_end_chance`, `corridor_segment_chance` + `corridor_coverage_min/max_percent`, `room_chance` + `room_coverage_min/max_percent`, `min_distance_to_same` (cell-to-cell), `density_min` / `density_max` (sprites per placed cell — (1, 1) = centred single sprite, > 1 = multi-sprite cluster), `jitter_radius` (sub-cell scatter as fraction of CELL_SIZE), `scale_min` / `scale_max` (per-sprite world-height variance). All three chances default to 0.0 — an empty spawn is a no-op. Density / jitter / scale all default to "centred single sprite at data world_height" so the original "one object per tile" look is preserved until a designer opts into multi-sprite.
4. ✅ `BiomeData.scenery_spawns: Array[ScenerySpawn]` exposed for biome configs.
5. ✅ `GridCell.scenery: SceneryInstance` — fifth parallel slot alongside `object`, `trap`, `spinner`, `teleporter`. `is_blocked` now also considers non-walkable scenery. Multi-sprite cells (`density > 1`) keep ONE pointer here (the first instance placed); the additional sprites only live in the flat `LevelGenerator.scenery` list.
6. ✅ `LevelGenerator._place_scenery()` runs IMMEDIATELY BEFORE `_place_fillers()` — last among walkable-area passes, so every other system's cells (chests / levers / doors / traps / spinners / teleporters / items / entrance / exit / projectile-trap plates + firing lines) are known and excluded. Three additive passes per spawn: dead-end (per cell), corridor (per segment from `_detect_corridor_segments`, coverage % of eligible cells), room (per `_room_rects`, coverage % of eligible cells). `_try_commit_scenery_at` rolls `density` once per placed cell and emits that many SceneryInstance entries with their own sub-cell jitter offsets + scale rolls. `_cell_eligible_for_scenery` excludes projectile-trap pressure-plate cells for ALL scenery, and every cell on a launcher's projectile path (`_projectile_path_cells`) for non-walkable scenery — a tree on the firing line would block the corridor / room the projectile crosses. Non-walkable scenery uses the same `_bfs_walkable_from(entrance_pos)` + `_placement_preserves_reachability` BFS guard chests already use — checked ONCE per cell since the first sprite flips `is_blocked` and subsequent sprites can't change reachability. `min_distance_to_same` graceful-degrades by 1 down to 0 when a coverage target can't be reached at the requested spread.
7. ✅ `DungeonView._build_scenery()` — separate render path under a new `SceneryRoot` Node3D. One `Sprite3D` per `SceneryInstance` (no Area3D — scenery is never clickable). `pixel_size = (data.world_height * inst.scale) / texture_height` so per-sprite scale changes the on-screen size. `_scenery_position(inst)` combines cell centre + lean offset + sub-cell jitter (`inst.cell_offset × CELL_SIZE`). `_scenery_sprites: Dictionary[SceneryInstance → Sprite3D]` powers `_refresh_scenery_positions()` for `lean_toward_player` updates on player turn — keyed by instance (not cell) so multi-sprite cells refresh all their sprites independently.
8. ✅ `MapPopup` — non-walkable scenery cells render as wall: the cell's floor rect is skipped (parchment background shows through, matching real walls) and `_has_wall(x, y)` returns true for the cell so neighbouring floor cells draw a wall line on their shared border. Walkable scenery gets no map marker (the player can step through it, so it's just floor).
9. ✅ Tests:
   - Unit (`test_scenery_data`): defaults, walkable flag, `get_display_name` / `get_display_description`.
   - Unit (`test_scenery_spawn`): defaults (no-op, single centred sprite), `uses_*` predicates, edge cases when coverage is zeroed despite a non-zero chance, `sample_density` (range, clamp to ≥ 1, inverted-range clamp), `sample_scale` (endpoints, strictly positive on degenerate input, inverted-range clamp).
   - Unit (`test_scenery_instance`): `create` (with and without offset/scale), `blocks_movement()` mirrors `walkable`, defensive null-data handling.
   - Integration (`test_level_generator_scenery`): empty pool no-op, default spawn no-op, room pass with full coverage fills rooms, dead-end pass with chance 1.0 fills every dead-end, walkable scenery never overlaps chests, non-walkable scenery never on item / entrance / exit cells, walkable scenery DOES land on entrance/exit (proves the exclusion isn't over-aggressive), non-walkable scenery preserves entrance↔exit reachability, `density > 1` emits N sprites per cell sharing the same cell coord, default density emits one centred sprite (cell_offset == 0, scale == 1.0), non-walkable + multi-sprite cells stay blocked + reachable, no scenery on projectile-trap plate cells, non-walkable scenery never on a projectile firing line.
10. ⏳ Manual asset + biome wiring (developer): pick a tree PNG (or reuse `res://assets/textures/biomes/sparse_forest/fillers/tree01.png` as a placeholder), create `res://assets/scenery/forest_tree.tres` (`SceneryData`, `walkable = false`, `world_height ≈ 3.5`, `lean_toward_player ≈ 1.2`), add a `ScenerySpawn` entry to `forest.tres` (e.g. `room_chance = 0.5`, `room_coverage_min_percent = 10.0`, `room_coverage_max_percent = 25.0`, `min_distance_to_same = 2`) to validate placement in-engine. For multi-sprite per cell try `density_min = 2`, `density_max = 4`, `jitter_radius = 0.3`, `scale_min = 0.85`, `scale_max = 1.15` on a flower spawn.
