...
6. ✅ `LevelGenerator._place_scenery()` runs IMMEDIATELY BEFORE `_place_fillers()` — last among walkable-area passes, so every other system's cells (chests / levers / doors / traps / spinners / teleporters / items / entrance / exit) are known and excluded. Three additive passes per spawn: dead-end (per cell), corridor (per segment from `_detect_corridor_segments`, coverage % of eligible cells), room (per `_room_rects`, coverage % of eligible cells). `_try_commit_scenery_at` rolls `density` once per placed cell and emits that many SceneryInstance entries with their own sub-cell jitter offsets + scale rolls. Non-walkable scenery uses the same `_bfs_walkable_from(entrance_pos)` + `_placement_preserves_reachability` BFS guard chests already use — checked ONCE per cell since the first sprite flips `is_blocked` and subsequent sprites can't change reachability. `min_distance_to_same` graceful-degrades by 1 down to 0 when a coverage target can't be reached at the requested spread.
