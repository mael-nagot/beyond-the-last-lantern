...
2. ✅ `SceneryInstance.gd` RefCounted — `data`, `cell`, `blocks_movement()` mirrors `not data.walkable`. Consumed by `GridCell.is_blocked` (a tree cell behaves exactly like a chest cell).
3. ✅ `ScenerySpawn.gd` Resource — biome-level entry: `scenery`, `dead_end_chance`, `corridor_segment_chance` + `corridor_coverage_min/max_percent`, `room_chance` + `room_coverage_min/max_percent`, `min_distance_to_same`. All chances default to 0.0 — an empty spawn is a no-op.
4. ✅ `BiomeData.scenery_spawns: Array[ScenerySpawn]` exposed for biome configs.
5. ✅ `GridCell.scenery: SceneryInstance` — fifth parallel slot alongside `object`, `trap`, `spinner`, `teleporter`. `is_blocked` now also considers non-walkable scenery.
6. ✅ `LevelGenerator._place_scenery()` runs IMMEDIATELY BEFORE `_place_fillers()` — last among walkable-area passes, so every other system's cells (chests / levers / doors / traps / spinners / teleporters / items / entrance / exit) are known and excluded. Three additive passes per spawn: dead-end (per cell), corridor (per segment from `_detect_corridor_segments`, coverage % of eligible cells), room (per `_room_rects`, coverage % of eligible cells). Non-walkable scenery uses the same `_bfs_walkable_from(entrance_pos)` + `_placement_preserves_reachability` BFS guard chests already use, rolling back the cell's scenery slot if the placement strands a previously-reachable cell. `min_distance_to_same` graceful-degrades by 1 down to 0 when a coverage target can't be reached at the requested spread.
7. ✅ `DungeonView._build_scenery()` — separate render path under a new `SceneryRoot` Node3D. One `Sprite3D` per `SceneryInstance` (no Area3D — scenery is never clickable). `_scenery_sprites: Dictionary[Vector2i → Sprite3D]` powers `_refresh_scenery_positions()` for `lean_toward_player` updates on player turn (called from `set_initial_facing` + `rotate_camera_to` — same wiring chests use, since the lean axis is tied to facing).
8. ✅ Tests:
   - Unit (`test_scenery_data`): defaults, walkable flag, `get_display_name` / `get_display_description`.
   - Unit (`test_scenery_spawn`): defaults (no-op), `uses_*` predicates, edge cases when coverage is zeroed despite a non-zero chance.
   - Unit (`test_scenery_instance`): `create`, `blocks_movement()` mirrors `walkable`, defensive null-data handling.
