...
3. ✅ `_place_corridor_traps()` runs BEFORE the legacy scattered pass on the same spawn — modes are additive. Per-segment rules:
   ...
   - When the segment is shorter than `corridor_traps_per_run_min`, place as many cells as fit (both bounds are SOFT). Originally we skipped short segments to preserve the "at least N in a row" guarantee; developer playtest showed this left short corridors leading to dead-ends entirely bare, which reads as a placement bug. Better to under-deliver visibly than to leave gaps.
   - Pick a random eligible start cell and BFS-flood within the segment to gather up to N contiguous cells (N rolled in [min, max], clamped to eligible count)
...
###### Subtask B3 — Path-safety validator + chest/lever adjacency ✅ (code; awaits developer playtest)
1. ✅ `_validate_chest_lever_timed_adjacency()` runs after `_place_items` — for every chest and lever cell, verifies at least one walkable 4-adjacent cell is free of TIMED traps. Step traps adjacent are OK (one-shot, the player can wait off-cell after triggering). When violated, removes one timed trap from a 4-neighbour. Gives every chest/lever a foothold the player can stand on while waiting out cycles.
2. ✅ Bug fix bundled: trap placement now skips cells with floor items (`cell.items` non-empty). Floor keys placed by `_place_key_doors` land before the trap passes; without this, a trap could land on a key cell and damage the player every time they pick the key up. Applies to all three trap passes (corridor cluster, room density, scattered).
3. ✅ `_remove_trap_at(pos)` is the single trap-removal entry point — clears `cell.trap`, removes from `traps` flat list, removes from `_cluster_cells` if present.
4. ✅ Tests: integration coverage for chest/lever timed adjacency holds across multiple seeds with timed corridor + room density; traps avoid floor key cells across multiple seeds with key-door spawns enabled.
5. **Originally planned, removed**: a strict step-trap reachability validator (BFS from entrance treating step traps as walls; ensure exit + chests + levers + items reachable without crossing step traps). First implemented, then removed after developer playtest: under aggressive trap configs (high corridor + room density) the rule forced removal of 95%+ of step traps, leaving levels visually empty. Step traps damage but don't BLOCK movement — the player can always walk through with HP cost. The cost is a designed trade-off for the designer to tune via density, not a bug for the validator to fix.
6. **Deferred**: TIMED-trap rest-cell validator (every timed trap has a non-trap cell within K Manhattan). The Subtask B2 `room_max_distance_to_safe_cell` rule already covers in-room timed traps, and the chest/lever adjacency rule covers the most player-impactful case. Cross-corridor timed clusters can be handled if needed in a future polish pass.
