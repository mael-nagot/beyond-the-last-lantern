...
4. ✅ `BiomeData` additions: `outdoor_mode: bool`, `filler_spawns: Array[FillerSpawn]`, `sky_color: Color` (alpha > 0 = use as flat background), `sky_material: Material` (when set, wrapped in a Sky resource and used as the background — clean path to swap in a panorama texture later)
...
6. ✅ `DungeonView._build_mesh` skips wall quads and the ceiling pass when `biome.outdoor_mode = true`; `_build_fillers()` instantiates one `Sprite3D` per filler under `_fillers_root` with `BILLBOARD_FIXED_Y` (always faces camera, stays vertical). `_apply_biome_sky()` swaps the WorldEnvironment background per biome and restores the scene defaults when leaving an outdoor biome
