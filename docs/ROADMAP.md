...
###### Subtask B3 — Path-safety validators + chest/lever adjacency ✅ (code; awaits developer playtest)
1. ✅ `_validate_step_trap_reachability()` runs after `_place_items` — BFS from entrance treats STEP traps as walls; the rule is satisfied iff the exit, every chest, every lever, and every floor-item cell is reachable. Chests count as reached if the cell OR any 4-neighbour is reachable (chests may be blocking, so the player approaches from adjacent). When unsatisfied, the validator iteratively removes step traps on the BOUNDARY of the reachable region (preferring "bridge" traps with both reachable AND non-reachable walkable neighbours, fallback to any trap with a reachable neighbour) until the rule holds or no removable boundary trap remains. Bounded by `traps.size()` iterations.
2. ✅ `_validate_chest_lever_timed_adjacency()` runs immediately after — for every chest and lever cell, verifies at least one walkable 4-adjacent cell is free of TIMED traps. Step traps adjacent are OK (they're one-shot, the player can wait off-cell after triggering). When violated, removes one timed trap from a 4-neighbour. Gives every chest/lever a foothold the player can stand on while waiting out cycles.
3. ✅ Bug fix bundled: trap placement now skips cells with floor items (`cell.items` non-empty). Floor keys placed by `_place_key_doors` land before the trap passes; without this, a trap could land on a key cell and damage the player every time they pick the key up. Applies to all three trap passes (corridor cluster, room density, scattered).
4. ✅ `_remove_trap_at(pos)` is the single trap-removal entry point — clears `cell.trap`, removes from `traps` flat list, removes from `_cluster_cells` if present. Used by both validators.
5. ✅ Tests: integration coverage for exit always reachable via no-step-trap path under aggressive cluster + room density configs; chests reachable directly or via 4-neighbour; rule survives long cluster runs; chest/lever timed adjacency holds across multiple seeds with timed corridor + room density; traps avoid floor key cells across multiple seeds with key-door spawns enabled.
