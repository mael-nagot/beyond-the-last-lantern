...
1. ✅ `SecretWallSpawn.gd` Resource — `count_min`/`count_max`, `gate_mode: GateMode { NONE, ANY_CONTENT, LOOT_ONLY }` (default ANY_CONTENT), `min_distance_to_other_object`. NO ObjectData reference — wall textures come from the biome's regular pool so the secret wall is indistinguishable from any other corridor wall.
