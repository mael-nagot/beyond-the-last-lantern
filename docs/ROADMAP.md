...
2. ✅ `FillerSpawn.gd` Resource — biome pool entry: `fillers` array (uniform pick), `density_min/max` per blocked cell, `jitter_radius` (sub-cell scatter as a fraction of CELL_SIZE), `border_ring_depth` (cells beyond the grid edge to also seed for horizon depth), `scale_min/max` (per-instance size variety so every tree isn't the same height)
