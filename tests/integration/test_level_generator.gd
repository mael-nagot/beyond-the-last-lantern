...
func test_chest_lever_timed_adjacency_holds_after_validation() -> void:
	...
	# validation, every chest cell must have at least one 4-adjacent
	# walkable cell free of timed traps.
	for seed_n in [12, 23, 34, 45, 56]:
		var biome := _make_biome()
		biome.objects = [_make_object_spawn(_make_chest(), 4, 4, ObjectSpawn.PLACEMENT_ANY)]
		var trap_data := _make_trap_data(TrapData.Trigger.TIMED, 4)
		var spawn := TrapSpawn.new()
		...
		spawn.room_coverage_min_percent = 60.0
		...
		for x in range(gen.grid_width):
			for y in range(gen.grid_height):
				...
				var has_safe_neighbour: bool = false
				for d: Vector2i in [Vector2i(0, -1), Vector2i(0, 1), Vector2i(-1, 0), Vector2i(1, 0)]:
					var n: Vector2i = pos + d
					if n.x < 0 or n.x >= gen.grid_width or n.y < 0 or n.y >= gen.grid_height:
						continue
					...
					if ncell.trap != null and ncell.trap.data != null and ncell.trap.data.is_timed():
						continue
					has_safe_neighbour = true
					break
				assert_true(has_safe_neighbour,
					"seed %d: chest at %s has no walkable timed-trap-free neighbour" % [seed_n, pos])

func test_traps_avoid_cells_with_floor_keys() -> void:
	# Floor keys are placed by _place_key_doors BEFORE _place_traps,
	# so trap placement must skip cells that already hold items —
	# otherwise picking up the key triggers the trap. Sweep several
	# seeds with aggressive trap placement and verify no trap shares
	# a cell with floor items.
	for seed_n in [1, 2, 3, 4, 5, 6]:
		var biome := _make_biome()
		biome.key_door_spawns = [_make_key_door_spawn(_make_key_item(), _make_locked_door(), 3, 3)]
		var trap_data := _make_trap_data(TrapData.Trigger.STEP, 4)
		biome.trap_spawns = [_make_trap_spawn(trap_data, 15, 15, ObjectSpawn.PLACEMENT_ANY)]
		var gen := _make_generator(biome, seed_n)
		for inst in gen.traps:
			var cell: GridCell = gen.grid[inst.cell.x][inst.cell.y]
			assert_eq(cell.items.size(), 0,
				"seed %d: trap at %s shares cell with floor items %s" % [seed_n, inst.cell, cell.items])
